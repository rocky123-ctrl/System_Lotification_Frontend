...
export function DashboardLayout({ children }: DashboardLayoutProps) {
  ...
  const navigation = [
    ...
    if (isVendedor) return ["Venta", "Cotizaciones"].includes(item.name)
    return false
  })
  ...
  const showFinancieroSection = isSuperAdmin || isAdmin
  const moduloFinancieroNavigation = [
    ...
    if (isAdmin) return item.name === "Planillas de Pago"
    return false
  })

  const showClientesSection = isSuperAdmin || isAdmin || isVendedor
  const clientesNavigation = [
    ...
    if (isSuperAdmin || isAdmin) return true
    if (isVendedor) return item.name === "Registrar Cliente"
    return false
  })
                    ...
                    <div className="ml-6 mt-1 space-y-1">
                      {lotificacionesNavigation.map((item) => (
                        <Link
                          ...
                          href={item.href}
                          className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm font-medium transition-colors ${
                            pathname === item.href ? "bg-sidebar-primary text-sidebar-primary-foreground" : "text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground"
                          }`}
                        >
                          <item.icon className="h-4 w-4" />
                          {item.name}
